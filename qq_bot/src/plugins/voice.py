...
@speaker.handle()
async def speak(event: MessageEvent):
    line = str(event.get_plaintext()).replace("/说话 ", "")
    await speaker.send(MessageSegment.audio(path=voice_generate(line)))


@translator.handle()
async def translate(event: MessageEvent):
    line = str(event.get_plaintext()).replace("/翻译", "")
...
async def test(event: MessageEvent):
    await test_amr.send(MessageSegment.audio(path="voice/alice.amr"))
